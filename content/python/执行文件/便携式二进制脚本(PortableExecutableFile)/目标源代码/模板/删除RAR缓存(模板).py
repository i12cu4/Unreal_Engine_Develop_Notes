"""
RAR压缩包删除冗余文件工具（高效版）增强报告版 - 便携版
功能:
1. 直接删除RAR压缩包中的指定文件和文件夹，无需解压/重新压缩
2. 支持控制台、CSV或双重报告输出
3. 实时显示处理进度和结果
4. 自动生成带时间戳的CSV报告
5. 使用嵌入式WinRAR工具，无需安装
"""
...
# ===================== 嵌入式二进制数据 =====================
# 此处已声明变量(二进制)
# BIN_WINRAR BIN_RAR BIN_UNRAR BIN_RAREXT_DLL
...
class PortableRARTools:
    """
    便携式RAR工具管理器
    负责从嵌入式二进制数据中提取和管理WinRAR工具
    """
    
    def __init__(self):
        self.temp_dir = tempfile.mkdtemp(prefix="rar_tools_")
    ...
    def initialize_tools(self):
        ...
        try:
            ...
            if not self.rar_exe_path:
                print("错误: 无法提取Rar.exe")
                return False
            
            # 验证RarExt.dll是否存在
            if "RarExt.dll" not in self.extracted_files:
                print("警告: RarExt.dll未找到，某些功能可能受限")
    ...
    def get_rar_exe_path(self):
        """获取Rar.exe路径"""
        return self.rar_exe_path
...
def get_entries_to_delete(rar_path, rar_exe_path):
    """获取需要删除的条目列表（增强过滤版）"""
    try:
...
def process_rar(rar_path, rar_exe_path):
    """直接通过RAR命令删除条目"""
    entries = get_entries_to_delete(rar_path, rar_exe_path)
    if not entries:
        return 0, set()
...
def main():
    ...
    try:
        ...
        # 获取RAR工具路径
        rar_exe_path = tools_manager.get_rar_exe_path()
        if not rar_exe_path:
            print("错误: 无法获取RAR工具路径")
            wait_for_exit()
            return
            
        print(f"使用RAR工具: {rar_exe_path}")
        ...
        with tqdm(rar_files, desc="处理压缩包", unit="file", bar_format="{l_bar}{bar}| {n_fmt}/{total_fmt}") as pbar:
            for rar_path in pbar:
                try:
                    ...
                    # 调用处理函数
                    deleted_count, patterns = process_rar(rar_path, rar_exe_path)
                    success = True
                    error_msg = None
