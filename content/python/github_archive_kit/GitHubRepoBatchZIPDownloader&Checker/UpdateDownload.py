"""
GitHub 仓库批量 ZIP 下载工具（增强版：安全更新 + 全面重试 + 手动文件识别 + 旧版本自动清理）
...
class GitHubZipDownloader:
    ...
    def move_to_old_versions(self, file_path):
        """将文件移动到 old_versions 目录，重名时添加时间戳"""
        ...
        self.log_message(f"📦 已移动到旧版本目录: {dest_path}")

    def clean_old_versions_for_repo(self, owner, repo, current_full_sha):
        """
        扫描当前目录，找出同仓库但不同短SHA的 ZIP 文件，移动到 old_versions
        """
        short_sha_current = current_full_sha[:7]
        pattern = f"{owner}_{repo}@*.zip"
        zip_root = Path(self.script_dir)
        for zip_file in zip_root.glob(pattern):
            # 提取短 SHA：文件名中 @ 后面、.zip 前面的部分
            name = zip_file.name
            # 格式: owner_repo@xxxxxxxx.zip
            try:
                sha_part = name.split('@')[1].split('.zip')[0]
                if sha_part != short_sha_current:
    ...
    def process_url(self, url, index, total):
        ...
        while overall_retry <= self.max_retries:
            ...
            if os.path.exists(expected_zip_path):
                ...
                self.save_config_atomic()
                # 清理同仓库的其他旧版本文件
                self.clean_old_versions_for_repo(owner, repo, latest_sha)
                return True
    ...
    def run(self, input_file):
        self.log_message("=" * 60)
        self.log_message("🚀 GitHub ZIP 批量下载工具（增强版）")
